  ...
  compilation (events, compilation) {
    ...
    this.wire(
      ...
      (pluginArgs) => {
        ...
        extTpl = extTpl.replace('<!--template-extract-css-->', `<style>${extStyle ? '\${style?style:String.raw`' + extStyle + '`}' : '\${style ? style : extStyle}'}</style>`);
        ...
        if (extStyle) {
          extTpl = `<script>(function (g) { g._get_extract_template = function (dom, style) {function raw(callSite = {}, ...substitutions) {var template;try {template = Array.from(callSite.raw);} catch (e) {throw new TypeError('Cannot convert undefined or null to object');}return template.map((chunk, i) => {if (callSite.raw.length <= i) {return chunk;}return substitutions[i - 1] ? substitutions[i - 1] + chunk : chunk;}).join('');};return \`${extTpl}\`;} })(typeof window === 'undefined' ? {} : window)</script>`
        } else {
          extTpl = `<script>(function (g) { g._get_extract_template = function (dom, style) {var extStyle = '';if (!g.__TEMPLATE_EXTRACT_PLUGIN_STYLE__) g.__TEMPLATE_EXTRACT_PLUGIN_STYLE__ = {};for (var k in g.__TEMPLATE_EXTRACT_PLUGIN_STYLE__) {extStyle += g.__TEMPLATE_EXTRACT_PLUGIN_STYLE__[k]}return \`${extTpl}\`;}})(typeof window === 'undefined' ? {} : window)</script>`
        }
      }
    );
