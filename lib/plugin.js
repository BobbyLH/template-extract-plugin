  ...
  compilation (events, compilation) {
    ...
    this.wire(
      ...
      (pluginArgs) => {
        ...
        extTpl = extTpl.replace('<!--template-extract-css-->', `<style>${extStyle ? '\${style ? style :' + extStyle + '}' : '\${style ? style : extStyle}'}</style>`);
        ...
        if (extStyle) {
          extTpl = `<script>(function (g) { g._get_extract_template = function (dom, style) {return \`${extTpl}\`;} })(typeof window === 'undefined' ? {} : window)</script>`
        } else {
