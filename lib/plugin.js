  ...
  compilation (events, compilation) {
    ...
    this.wire(
      ...
      (pluginArgs) => {
        ...
        if (extStyle) {
          extTpl = `(function (g) { g._get_extract_template_ = function (dom, style) {function raw(callSite = {}, ...substitutions) {var template;try {template = Array.from(callSite.raw);} catch (e) {throw new TypeError('Cannot convert undefined or null to object');}return template.map((chunk, i) => {if (callSite.raw.length <= i) {return chunk;}return substitutions[i - 1] ? substitutions[i - 1] + chunk : chunk;}).join('');};return \`${extTpl}\`;} })(typeof window === 'undefined' ? {} : window)`
        } else {
          extTpl = `(function (g) { g._get_extract_template_ = function (dom, style) {var extStyle = '';if (!g.__TEMPLATE_EXTRACT_PLUGIN_STYLE__) g.__TEMPLATE_EXTRACT_PLUGIN_STYLE__ = {};for (var k in g.__TEMPLATE_EXTRACT_PLUGIN_STYLE__) {extStyle += g.__TEMPLATE_EXTRACT_PLUGIN_STYLE__[k]}return \`${extTpl}\`;}})(typeof window === 'undefined' ? {} : window)`
        }
      }
    );
      ...
      (pluginArgs) => {
        ...
        if (injectInJs) {
          ...
          if (!jsFiles || jsFiles.length === 0) {
            compilation.warnings.push(`[${PLUGIN_NAME}]: There is not any js file, The templates will inject in html file!`);
            pluginArgs.html = pluginArgs.html.replace(toReplace, replaceWith);
          } else {
            let fileIndex = 0;
            jsFiles.some((fileName, ind) => {
              if (~fileName.indexOf('chunk') || ~fileName.indexOf('main') || ~fileName.indexOf('index')) {
                fileIndex = ind;
                return true;
              }
              return false;
            });
            const injectFileName = jsFiles[fileIndex].split('?')[0];
            compilation.assets[injectFileName] = new ConcatSource(
              compilation.assets[injectFileName],
              extTpl
            );
          }
        } else {
