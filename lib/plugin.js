  ...
  compilation (events, compilation) {
    ...
    this.wire(
      ...
      (pluginArgs) => {
        ...
        extTpl = extTpl.replace('<!--template-extract-css-->', `<style>${extStyle ? extStyle : '\${extStyle}'}</style>`)
        ...
        if (extStyle) {
          extTpl = `<script>(function (g) { g._get_extract_template = function (dom) {return \`${extTpl}\`;} })(typeof window === 'undefined' ? {} : window)</script>`
        } else {
          extTpl = `<script>(function (g) { g._get_extract_template = function (dom) {var extStyle = '';if (!g.__TEMPLATE_EXTRACT_PLUGIN_STYLE__) g.__TEMPLATE_EXTRACT_PLUGIN_STYLE__ = {};for (var k in g.__TEMPLATE_EXTRACT_PLUGIN_STYLE__) {extStyle += g.__TEMPLATE_EXTRACT_PLUGIN_STYLE__[k]}return \`${extTpl}\`;}})(typeof window === 'undefined' ? {} : window)</script>`
        }
      }
    );
